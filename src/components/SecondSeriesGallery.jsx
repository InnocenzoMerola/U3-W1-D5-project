...
const SecondSeriesGallery = function () {
  const [movies, setMovies] = useState([]);

  const MY_KEY = "717484dc";

  const fetchThirdGall = () => {
    fetch(`http://www.omdbapi.com/?apikey=${MY_KEY}&type=series&s=horror`)
      .then((response) => {
        if (response.ok) {
  ...
  return (
    <>
      <h4 className="text-white">Horror series</h4>
...
export default SecondSeriesGallery;
